...
RendererBase::RendererBase(SDL_Window *window, int screenWidth, int screenHeight) {
	mWindow = window;
...
void RendererBase::beginFrame() {
	...
	glm::mat4 projMatrix = glm::ortho(0.0f, (float)mScreenWidth, (float)mScreenHeight, 0.0f, 0.0f, 1.0f);
...
Texture *RendererBase::getTexture(const std::string &filename) {
...
void RendererBase::freeUnreferencedTextures() {
...
SpriteBatcher *RendererBase::getSpriteBatcher(Texture *texture, SpriteBlendMode blendMode) {
	assert(texture != NULL);
